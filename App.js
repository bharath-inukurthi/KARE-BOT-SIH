...
// Helper to get and refresh Google access token
export async function getValidGoogleAccessToken() {
  try {
    let accessToken = await SecureStore.getItemAsync('googleAccessToken');
    let expiry = await SecureStore.getItemAsync('googleAccessTokenExpiry');
    let refreshToken = await SecureStore.getItemAsync('googleRefreshToken');
    ...
    console.log('Token validation check:', {
      ...
      currentTime: now,
      expiryTime: expiry ? Number(expiry) : null,
      timeUntilExpiry: expiry ? Number(expiry) - now : null,
      isTokenValid: accessToken && expiry && Number(expiry) > now + 60
    });
    
    // Check if current token is still valid (with 60 second buffer)
    if (accessToken && expiry && Number(expiry) > now + 60) {
    ...
    if (refreshToken) {
      ...
      try {
        ...
        console.log('Token refresh result:', {
          ...
          hasExpiresIn: !!tokenResult.expiresIn,
        ...
        if (tokenResult.accessToken) {
          ...
          if (tokenResult.expiresIn) {
            const newExpiry = now + tokenResult.expiresIn;
            await SecureStore.setItemAsync('googleAccessTokenExpiry', String(newExpiry));
            console.log('Updated access token expiry to:', newExpiry);
          }
          if (tokenResult.refreshToken) {
            await SecureStore.setItemAsync('googleRefreshToken', tokenResult.refreshToken);
        ...
        try {
          ...
          await SecureStore.deleteItemAsync('googleAccessTokenExpiry');
          await SecureStore.deleteItemAsync('googleRefreshToken');
          console.log('Cleared all tokens due to refresh failure');
        } catch (clearErr) {
    ...
    console.error('Error in getValidGoogleAccessToken:', error);
    return null;
  }
}
...
const AppContent = () => {
  ...
  const [_, response, promptAsync] = Google.useAuthRequest({
    clientId: 'your client id',
    androidClientId: 'your credentials',
    iosClientId: 'your credentials',
      ...
      async (event, session) => {
        ...
        if (session?.user) {
          ...
          if (userEmail && !userEmail.endsWith('@klu.ac.in') && userEmail !== 'test.klu@gmail.com') {
            ...
            // Clear the Google access token when signed out due to invalid domain
            try {
              await SecureStore.deleteItemAsync('googleAccessToken');
              console.log('Cleared Google access token due to invalid email domain during Google sign-in');
            } catch (error) {
              console.error('Error clearing Google access token during Google sign-in:', error);
            }
            setUser(null);
          } else {
          ...
          try {
            ...
            // Also clear the Google access token when signed out
            await SecureStore.deleteItemAsync('googleAccessToken');
            console.log('Cleared user UUID and Google access token during sign out');
          } catch (error) {
            console.error('Error clearing user UUID and Google access token during sign out:', error);
          }
        }
    ...
    const getInitialSession = async () => {
      ...
      if (session?.user) {
        ...
        if (userEmail && !userEmail.endsWith('@klu.ac.in') && userEmail !== 'test.klu@gmail.com') {
          ...
          await supabase.auth.signOut();
          setUser(null);
          // Clear the Google access token when signed out due to invalid domain in initial session
          try {
            await SecureStore.deleteItemAsync('googleAccessToken');
            console.log('Cleared Google access token due to invalid email domain in initial session');
          } catch (error) {
            console.error('Error clearing Google access token in initial session:', error);
          }
        } else {
          setUser(session.user);
  ...
  useEffect(() => {
    ...
    const handleGoogleSignIn = async () => {
      try {
        ...
        // Store the access and refresh tokens for Gmail API access
        if (response.authentication?.accessToken) {
          await SecureStore.setItemAsync('googleAccessToken', response.authentication.accessToken);
        }
        if (response.authentication?.refreshToken) {
          await SecureStore.setItemAsync('googleRefreshToken', response.authentication.refreshToken);
        }
        if (response.authentication?.expiresIn) {
          const now = Math.floor(Date.now() / 1000);
          await SecureStore.setItemAsync('googleAccessTokenExpiry', String(now + Number(response.authentication.expiresIn)));
        }
