...
const ProfileScreen = () => {
  ...
  const [feedbackSuccess, setFeedbackSuccess] = React.useState(null); // null | true | false
...
useEffect(() => {
  ...
  const handleSubmitFeedback = async () => {
    ...
    try {
      ...
      const response = await fetch('https://karebot-low-traffic.onrender.com/feedbback', {
        method: 'POST',
        ...
        >
          ...
          <Dialog.Content>
            ...
            {feedbackSuccess === true ? (
              ...
              <>
                <Text style={{ color: isDarkMode ? COLORS.secondary : COLORS.text, marginBottom: 10 }}>
                  Your feedback helps us improve KARE Bot! Please share your thoughts below.
                </Text>
                <TextInput
                  style={{
                    minHeight: 80,
                    borderColor: feedbackError ? COLORS.error : (isDarkMode ? COLORS.primaryLight : COLORS.primary),
                    borderWidth: 1,
                    borderRadius: 8,
                    padding: 10,
                    color: isDarkMode ? COLORS.secondary : COLORS.text,
                    backgroundColor: isDarkMode ? COLORS.backgroundDark : COLORS.background,
                    marginBottom: 8,
                  }}
                ...
                />
                {feedbackError ? (
                  <Text style={{ color: COLORS.error, marginBottom: 8 }}>{feedbackError}</Text>
                ) : null}
              </>
            )}
          </Dialog.Content>
          <Dialog.Actions>
            ...
            >
              {feedbackSuccess === true ? 'Close' : 'Cancel'}
            </Button>
            <Button
            ...
            >
              {isSendingFeedback ? (
                <Text style={{ color: COLORS.primary, fontWeight: 'bold' }}>Sending...</Text>
              ) : (
